...
class DirectExecutionService {
  ...
  const harness = `\n\n// === Codester Harness (auto-generated) ===\n(function(){\n  function __parseInputs(s) {\n    try {\n      const data = JSON.parse(s);\n      if (Array.isArray(data)) {\n        // If it's a flat numeric array (e.g., [1,2,3]), treat as a single argument\n        const flatNums = data.every(x => typeof x === 'number');\n        return flatNums ? [data] : data;\n      }\n    } catch (_) {}\n    const vals = [];\n    for (const ln of s.replace(/\\r/g, '').split('\\n').map(x => x.trim()).filter(Boolean)) {\n      try { vals.push(JSON.parse(ln)); }\n      catch (_) {\n        const n = Number(ln);\n        if (!Number.isNaN(n)) vals.push(n); else vals.push(ln);\n      }\n    }\n    return vals;\n  }\n\n  // Linked list helpers\n  function ListNode(val, next){ this.val = val; this.next = next || null; }\n  function fromArray(arr){ const d=new ListNode(0); let c=d; for(const v of (arr||[])){ c.next=new ListNode(v); c=c.next; } return d.next; }\n  function toArray(head){ const out=[]; let cur=head; while(cur){ out.push(cur.val); cur=cur.next; } return out; }\n\n  const __RAW = ${JSON.stringify(String(testInputString || ''))};\n  const __PARSED = __parseInputs(__RAW);\n  let __ARGS = __PARSED;\n  let __LL_MODE = false;\n  let __LL_EMPTY = false;\n  // Auto-convert single array arg into ListNode for common signatures\n  if (Array.isArray(__ARGS) && __ARGS.length === 1 && Array.isArray(__ARGS[0])) {\n    __LL_MODE = true;\n    __LL_EMPTY = Array.isArray(__ARGS[0]) && __ARGS[0].length === 0;\n    __ARGS = [ fromArray(__ARGS[0]) ];\n  }\n\n  function __unique(arr){ return Array.from(new Set(arr.filter(Boolean))); }\n\n  function __getCandidateNames(){\n    const primary = ${JSON.stringify(funcName)};\n    const base = [];\n    // Strong preference for reverseList when present\n    base.push('reverseList');\n    // Add known common names then the detected primary\n    base.push(primary);\n    base.push('twoSum');\n    const names = __unique(base);\n    // If class Solution exists, merge its prototype method names (excluding constructor)\n    if (typeof Solution === 'function') {\n      try {\n        for (const n of Object.getOwnPropertyNames(Solution.prototype)) {\n          if (n !== 'constructor' && typeof Solution.prototype[n] === 'function') {\n            if (!names.includes(n)) names.push(n);\n          }\n        }\n      } catch (_) {}\n    }\n    return names;\n  }\n\n  function __tryCallName(name) {\n    // Prefer instance method if class Solution exists\n    if (typeof Solution === 'function') {\n      try {\n        const inst = new Solution();\n        if (name && typeof inst[name] === 'function') {\n          return { ok: true, val: inst[name](...__ARGS) };\n        }\n      } catch (_) { /* fall back */ }\n    }\n    // Try global scope\n    try {\n      const fn = (name && (globalThis[name] || eval(name)));\n      if (typeof fn === 'function') return { ok: true, val: fn(...__ARGS) };\n    } catch (_) {}\n    return { ok: false };\n  }\n\n  function __resolveAndCall(){\n    const candidates = __getCandidateNames();\n    for (const name of candidates) {\n      if (!name) continue;\n      const r = __tryCallName(name);\n      if (r.ok) {\n        // If result is undefined and there are more candidates, keep trying (likely wrong method)\n        if (typeof r.val === 'undefined' && candidates.length > 1) continue;\n        return r.val;\n      }\n    }\n    throw new Error('Function not found: ' + candidates.join(', '));\n  }\n\n  let __result;\n  try {\n    __result = __resolveAndCall();\n  } catch (e) {\n    console.error(String(e));\n    throw e;\n  }\n\n  try {\n    // If linked-list mode and input was empty and result is null/undefined, print []\n    if (__LL_MODE && __LL_EMPTY && (__result === null || typeof __result === 'undefined')) {\n      console.log('[]');\n      return;\n    }\n    if (__result && typeof __result === 'object' && 'val' in __result && 'next' in __result) {\n      console.log(JSON.stringify(toArray(__result)));\n    } else {\n      console.log(JSON.stringify(__result));\n    }\n  } catch (_) {\n    try { console.log(JSON.stringify(toArray(__result))); } catch { console.log(String(__result)); }\n  }\n})();\n`;
  ...
  buildCppExecutable(userCode, testInputString) {
    ...
    let funcName = 'twoSum';
    // Prefer reverseList if it appears anywhere in user code
    if (/\breverseList\s*\(/.test(userCode)) {
      funcName = 'reverseList';
    } else {
      // Try to detect method name by return type including ListNode*
      const m = userCode.match(/class\s+Solution[\s\S]*?(?:ListNode\s*\*|vector\s*<[^>]+>\s*|int\s+|long\s+|bool\s+|string\s+)(\w+)\s*\(/i);
  ...
  // Always provide a concrete ListNode definition before user code so method bodies see the complete type
