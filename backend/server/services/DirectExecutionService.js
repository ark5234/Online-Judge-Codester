...
class DirectExecutionService {
  ...
  buildJavaExecutable(userCode, testInputString) {
    ...
    let sanitized = userCode
      ...
      .replace(/public\s+(class|interface|enum)\s+(?!Main\b)/g, '$1 ');
    ...
    // Place sanitized user code first, then our harness (which has no imports)
    return `${sanitized}\n${harness}`;
  }
  ...
  const needsListNode = !/(struct|class)\s+ListNode\b/.test(userCode);
